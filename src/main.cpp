...
static const int64 nGenesisBlockSubsidy = 1 * COIN;
static const int64 nPremineBlockSubsidy = 392160 * COIN; // Premined 3% for Developement (Original)
static const int64 nBlockSubsidyEnd = 0.0002 * COIN; // ~367.92 Annually Mined Chain Interest
                                                           //(Interest Dust Created for Network Stability)
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 0) {
        return  nGenesisBlockSubsidy;
    }

    //Mined 3% and Checkpoint Secured for Network Stability (Original).
    else if(nHeight >= 1 && nHeight < 6) {
        nSubsidy = nPremineBlockSubsidy;
    }

    // Block Subsidy End (HardCap Enforecment 2.0 by CryptoCoderz :) Mineout @ 65,360,000)
   else if (nHeight >= 297091) {
       nSubsidy = nBlockSubsidyEnd;
   }
